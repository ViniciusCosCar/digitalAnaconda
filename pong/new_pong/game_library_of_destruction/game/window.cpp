...
CollisionInfo Window::getCollisionInfo(Pos current_pos, Pos d) {
  ...
  if (d.y * d.x != 0) {
    ...
    if ((target.y = map[y + d.y + BORDERS_WIDTH][x + BORDERS_WIDTH]) !=
        NO_TARGET)
      direction.y = (d.y > 0) ? DOWN : UP;
    if ((target.x = map[y + BORDERS_WIDTH][x + d.x + BORDERS_WIDTH]) !=
        NO_TARGET)
...
bool Window::move(GameObject &obj, Pos d) {
  ...
  // Collision
  if (collision.target.yx != NO_TARGET || collision.target.y != NO_TARGET ||
